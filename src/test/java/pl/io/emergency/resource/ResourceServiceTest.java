...
public class ResourceServiceTest {
    ...
    public void findResourcesByDestinationIdTest() {
        ...
        // Zamockowanie repozytorium
        Mockito.when(resourceRepositorium.findByDestinationId(destinationId)).thenReturn(Arrays.asList(resource1, resource2));

        // Wywołanie metody serwisowej
        List<ResourceDto> result = resourceService.findResourcesByDestinationId(destinationId);

        // Sprawdzamy, czy wynik jest zgodny z oczekiwaniami
        assertEquals(2, result.size(), "Powinna być 2 zasoby dla destinationId=1");
        ...
        // Weryfikujemy, czy repozytorium zostało wywołane z odpowiednim parametrem
        Mockito.verify(resourceRepositorium, Mockito.times(1)).findByDestinationId(destinationId);
    }
